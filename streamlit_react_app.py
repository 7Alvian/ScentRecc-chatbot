...
st.title("ScentRec ReAct Chatbot")
st.caption('''Sebut aja aroma favorit kamu, nanti aku pilihin yang paling cocok 🔥🤝
No more parfum salah pilih!''')
...
if ("agent" not in st.session_state) or (getattr(st.session_state, "_last_key", None) != google_api_key):
    try:
        ...
        st.session_state.agent = create_react_agent(
            ...
            prompt='''You are a perfume recommendation assistant. 
                        Your task is to recommend perfumes based on the user's preferences and budget. 
                        Ask relevant questions if needed such as preferred scent profile, longevity, projection, season, gender expression, budget, and availability in Indonesia.

                        Provide recommendations separated into categories if requested:
                        - Lokal (Indonesia)
                        - Middle Eastern
                        - Designer
                        - Niche

                        For each perfume recommendation, always include:
                        - Nama parfum
                        - Tipe aroma (fresh, sweet, amber, oud, citrus, etc.)
                        - Kesan utama (first impression)
                        - Longevity & projection (singkat saja)
                        - Kisaran harga dan di mana biasa tersedia (ID store/e-commerce)
                        - Alternatif yang lebih murah/lebih mahal jika cocok

                        Always answer in Bahasa Indonesia unless the user requests otherwise.
